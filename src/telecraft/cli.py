...
def _need_env(name: str) -> str:
    ...
    if not v:
        raise SystemExit(f"Missing {name}. Export {name} before running this command.")
    return v
    ...
    *,
    ...
    while not stop.is_set():
        ...
        try:
            ...
            print(
                f"Warning: keepalive ping failed while waiting for input: {type(e).__name__}: {e}"
            )
...
def _resolve_runtime_network(args: argparse.Namespace) -> tuple[str, str]:
    ...
    try:
        ...
        if network_arg:
            print(
                "Warning: --network is deprecated; runtime determines network. Use --runtime prod."
            )
        ...
        if runtime == "prod":
            require_prod_override(
                ...
                example=(f"TELECRAFT_ALLOW_PROD=1 telecraft {cmd} --runtime prod --allow-prod ..."),
            )
    ...
    *,
    ...
    if session_raw is None:
        try:
            ...
            # Login is an explicit request to create authorization state. A
            # dangling current pointer must not select another account, but it
            # also must not prevent recreating the default session lane.
            session = str(
                default_session_path(
                    session_paths,
                    dc=int(args.dc),
...
async def _cmd_login_bot(args: argparse.Namespace) -> int:
    ...
    token = await _resolve_bot_token(args)
    session = ctx.session_path
...
async def _cmd_me(args: argparse.Namespace) -> int:
    ...
    client = MtprotoClient(
        ...
        session_path=session,
        init=ClientInit(api_id=api_id, api_hash=api_hash),
        trust_legacy_updates_state=bool(args.trust_legacy_updates_state),
    )
    await client.connect(timeout=args.timeout)
    try:
...
def _parse_peer_arg(raw: str) -> PeerRef:
    """
    CLI helper:
    ...
    for prefix in ("user:", "chat:", "channel:"):
        if s.startswith(prefix):
            ...
            return cast(PeerRef, (pt, int(rest)))
    return s
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    def add_common(sp: argparse.ArgumentParser, *, default_session_kind: str = "user") -> None:
        ...
        sp.add_argument(
            "--trust-legacy-updates-state",
            action="store_true",
            default=False,
            help=("One-time migration: trust an unbound v1 updates checkpoint for this session"),
        )
        sp.add_argument(
            "--session-kind",
            choices=["user", "bot"],
