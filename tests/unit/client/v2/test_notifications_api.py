...
def test_notifications__get__delegates_to_raw() -> None:
    raw = _RawNotifications()
    client = Client(raw=raw)
    out = asyncio.run(client.notifications.get(NotifyTarget.users(), timeout=5.0))
